...
class UnstructuredPruner():
    ...
    def update_threshold(self):
        ...
        for name, sub_layer in self.model.named_sublayers():
            if not self._should_prune_layer(sub_layer):
                continue
            for param in sub_layer.parameters(include_sublayers=False):
                if 'pact' in param.name or 'accum' in param.name or 'scale' in param.name or 'state' in param.name: continue
    ...
    def total_sparse(model):
        ...
        values = 0
        for name, sub_layer in model.named_sublayers():
            if type(sub_layer).__name__.split('.')[-1] in NORMS_ALL: continue
            for param in sub_layer.parameters(include_sublayers=False):
    ...
    def _should_prune_layer(self, layer):
        ...
        cond_quant = layer.full_name().replace('quantized_', '') not in self.skip_params
        return should_prune and cond_quant
