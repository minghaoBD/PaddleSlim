...
from unstructured_pruner import UnstructuredPruner
sys.path.append(
...
add_arg('num_workers',     int, 8,             "number of workers when loading dataset.")
...
add_arg('use_pact', bool, True, "whether to use PACT for activations. Default: True")
# yapf: enable
...
def compress(args):
    ...
    teacher_model = resnet50(num_classes=class_dim, pretrained=True)
    ...
    def train(epoch):
        ...
        for batch_id, data in enumerate(train_loader):
            ...
            logits_teacher = teacher_model(x_data)
            logits_teacher = F.softmax(logits_teacher / 1.0)
            logits_teacher.stop_gradient = True
            logits_student = F.softmax(logits / 1.0)

            loss = F.cross_entropy(logits, y_data)
            distill_loss = paddle.mean(F.cross_entropy(logits_student, logits_teacher, soft_label=True))
            loss += distill_loss
